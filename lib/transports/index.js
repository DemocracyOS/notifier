...
function createTransport (config) {
  if (config && config.service) {
    ...
    Object.keys(config).forEach(function (key) {
      if (key === 'nodemailer') return
      if (config.nodemailer && ~Object.keys(config.nodemailer).indexOf(key)) {
        opts[key] = config.nodemailer[key]
      } else {
        opts[key] = config[key]
      }
    })
    Object.keys(config.nodemailer).forEach(function (key) {
      if (!~Object.keys(config).indexOf(key)) {
        opts[key] = config.nodemailer[key]
      }
    })
  }
  log('Initializing nodemailer API client', opts)
