...
module.exports = function (opts) {
  db = db(opts.mongoUrl)
  opts.eventsAndJobs['comment-reply'] = jobName
  ...
  opts.agenda.define(jobName, function (job, done) {
    ...
    if (data.reply.author.id === data.comment.author.id) {
      return done(new Error('Ingoring job %s - Reply author equals comment author', jobName))
    }

    db.users.findOne({ _id: ObjectId(data.comment.author.id) }, function (err, commentAuthor) {
      log('Found comment author: %j', commentAuthor)

      if (err) return done(err)
      if (!commentAuthor) return done(new Error('user not found for comment author with id \'%s\'', data.comment.author.id))

      // Don't notify author if they chose not to be notified
      if (commentAuthor.notifications && !commentAuthor.notifications.replies) {
        log('Ingoring job "%s" - Comment author notifications are off for this event', jobName)
        return done()
      }

      log('about to find reply author for ID: %s', data.reply.author.id)
      db.users.findOne({ _id: ObjectId(data.reply.author.id) }, function(err, replyAuthor) {
        debugger;
        log('Found reply author: %j', replyAuthor)
        if (err) return done(err)
        if (!replyAuthor) return done(new Error('user not found for reply author with id "%s"', data.reply.author.id))
